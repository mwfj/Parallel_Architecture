...
__global__
void matrixMulKernal(float *C, const float *A, const float *B, unsigned int hA, unsigned int wA, unsigned int wB){
    /* 
    *  A[hA][wA]
    *  B[wA][wB]
    *  C[hA][wB]
    */
    ...
    if (threadId < hA * wB) {
        ...
        int row = threadId / wB;
        int column = threadId % wB;

        C[threadId] = 0;
                
        for(int i = 0; i < wA; i++)
        {
            C[threadId] += A[row * wA + i] * B[i * wB + column];
        }
        
    } 
    else
    {
        printf("The current threadId has exceeded the range of matrix C\n");
    }
...
int main(int argc, char const *argv[])
{
...

    {
        ...
        for (int j = 0; j < nIter; j++)
        {
            ...
            matrixMulKernal<<<grid,threads>>>(d_C,d_A,d_B,matrix_size.uiHA,matrix_size.uiWA,matrix_size.uiWB);
